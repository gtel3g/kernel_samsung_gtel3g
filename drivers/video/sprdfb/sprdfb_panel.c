...
static int __init lcd_id_get(char *str)
{
	...
	pr_info("[LCD] %s, lcd_id from bootloader = 0x%x\n",
...
static int __init lcd_base_get(char *str)
{
	...
	pr_info("[LCD] %s, fb_base from bootloader = 0x%x\n",
...
static bool panel_check(struct panel_cfg *cfg)
{
	...
	if (!cfg || !cfg->panel) {
		pr_err("[LCD] %s, no cfg or no panel\n", __func__);
		return false;
	}

	pr_debug("[LCD] %s, %s, id:0x%x, type:%d\n", __func__,
...
int panel_init(struct sprdfb_device *dev)
{
	...
	if (!dev->panel->if_ctrl->panel_if_init(dev)) {
		pr_err("[LCD] %s, panel_if_init failed\n", __func__);
		return -1;
	}
...
static struct panel_spec *adapt_panel_from_uboot(uint16_t dev_id)
{
	...
	if (!lcd_id_from_uboot) {
		pr_err("[LCD] %s, no panel from bootloader\n", __func__);
		return NULL;
	}
	...
	list_for_each_entry(cfg, panel_list, list) {
		if (lcd_id_from_uboot == cfg->lcd_id) {
			pr_info("[LCD] %s, panel found (id:0x%x)\n",
					__func__, cfg->lcd_id);
			return cfg->panel;
		}
	}
	pr_err("[LCD] %s, panel not found (id:0x%x)\n",
			__func__, lcd_id_from_uboot);
...
static struct panel_spec *adapt_panel_from_readid(struct sprdfb_device *dev)
{
	...
	list_for_each_entry(cfg, panel_list, list) {
		...
		pr_debug("[LCD] %s, try to find panel 0x%x\n",
				__func__, cfg->lcd_id);
	...
	if (dummy_cfg) {
		...
		if (id == dummy_cfg->lcd_id) {
			pr_info("[LCD] %s, dummy panel found (id:0x%x)\n",
...
bool sprdfb_panel_get(struct sprdfb_device *dev)
{
	...
	if (panel) {
		...
		panel_init(dev);
		pr_info("[LCD] %s, got panel\n", __func__);
		return true;
	}

	pr_info("[LCD] %s, failed to get panel\n", __func__);
...
uint32_t sprdfb_panel_ESD_check(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_EDPI == dev->panel_if_type) {
		...
		pr_debug("[LCD] sprdfb: after wait (%d)\n", result);
	...
	if (!dev->enable) {
		pr_err("[LCD] %s, device not enabled\n", __func__);
		return 0;
	}
	...
	if (!result) {
		...
		if (!if_status) {
			pr_info("[LCD] %s, need reset panel(%d, %d, %d)\n", __func__,
					dev->check_esd_time,
					dev->panel_reset_time,
					dev->reset_dsi_time);
			panel_reset(dev);
			if (!dev->enable) {
				pr_err("[LCD] %s, device not enabled after reset\n",
						__func__);
				return 0;
			}
		...
		pr_debug("[LCD] %s panel has been reset\n", __func__);
		return 1;
	}
	return 0;
}
...
void sprdfb_panel_resume(struct sprdfb_device *dev, bool from_deep_sleep)
{
	...
	pr_info("[LCD] %s, + enable:%d, from_deep_sleep:%d\n",
...
int sprdfb_panel_register(struct panel_cfg *cfg)
{
	...
	pr_info("[LCD] %s, register panel(%s, id:0x%x) on %s lcd list\n", __func__,
